...
*Setup instructions for a gitian build of QUISH using a VM or physical system.*

Gitian is the deterministic build process that is used to build the QUISH
...
to the QUISH GitHub release page.
...
- [Building QUISH](#building-QUISH-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/QUISH-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/QUISH-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/QUISH-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/QUISH-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/QUISH-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/QUISH-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/QUISH-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@QUISH.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/QUISH-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/QUISH-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/QUISH-osx-*-build.assert
```
