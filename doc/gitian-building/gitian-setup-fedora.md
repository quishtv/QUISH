...
The build script we'll be using is contained in the QUISH github repository ([contrib/gitian-build.py](https://github.com/QUISH-project/QUISH/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the QUISH repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/QUISH-Project/QUISH/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, QUISH-detached-sigs, and QUISH GitHub repos.
