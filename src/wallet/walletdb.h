...
public:
    ...
    bool WriteZQTVSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZQTVSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZQTVSeed_deprecated(uint256& seed);
    bool EraseZQTVSeed();
    bool EraseZQTVSeed_deprecated();

    bool WriteZQTVCount(const uint32_t& nCount);
    bool ReadZQTVCount(uint32_t& nCount);
