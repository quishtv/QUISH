...
CZQTVStake::CZQTVStake(const libzerocoin::CoinSpend& spend)
{
...
int CZQTVStake::GetChecksumHeightFromMint()
{
...
int CZQTVStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZQTVStake::GetChecksum()
{
    return nChecksum;
}

// The zQTV block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZQTVStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZQTVStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zQTV staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZQTVStake::GetUniqueness()
{
    //The unique identifier for a zQTV is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZQTVStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZQTVStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zQTV that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZQTVOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zQTV output", __func__);
    vout.emplace_back(outReward);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZQTVOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zQTV output", __func__);
...
//!QTV Stake
bool CQTVStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CQTVStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CQTVStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CQTVStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CQTVStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CQTVStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CQTVStake::GetUniqueness()
{
    //The unique identifier for a QTV stake is the outpoint
...
CBlockIndex* CQTVStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
