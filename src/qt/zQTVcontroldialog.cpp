...
#include "zQTVcontroldialog.h"
#include "ui_zQTVcontroldialog.h"
...
std::set<std::string> ZQTVControlDialog::setSelectedMints;
std::set<CMintMeta> ZQTVControlDialog::setMints;

bool CZQTVControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZQTVControlDialog::COLUMN_DENOMINATION || column == ZQTVControlDialog::COLUMN_VERSION || column == ZQTVControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZQTVControlDialog::ZQTVControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZQTVControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zQTV Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZQTV->setProperty("cssClass", "text-main-purple");
    ui->labelZQTV_int->setProperty("cssClass", "text-main-purple");
...
ZQTVControlDialog::~ZQTVControlDialog()
{
    delete ui;
}

void ZQTVControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZQTVControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZQTVControlWidgetItem* itemDenom(new CZQTVControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZQTVControlWidgetItem *itemMint = new CZQTVControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zQTV.";
            else if (!mint.isSeedCorrect)
                strReason = "The zQTV seed used to mint this zQTV is not the same as currently hold in the wallet";
            else
...
void ZQTVControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZQTVControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZQTV_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZQTVControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZQTVControlDialog::GetSelectedMints()
{
...
void ZQTVControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
