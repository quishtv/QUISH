...
#include "qt/QUISH/forms/ui_addresseswidget.h"
#include "qt/QUISH/addresslabelrow.h"
#include "qt/QUISH/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(QUISHGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a QUISH address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress QTVAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(QTVAdd.Get(), label.toUtf8().constData(),
                QTVAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
