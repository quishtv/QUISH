...
#include "qt/QUISH/send.h"
#include "qt/QUISH/forms/ui_send.h"
#include "qt/QUISH/addnewcontactdialog.h"
#include "qt/QUISH/qtutils.h"
#include "qt/QUISH/sendchangeaddressdialog.h"
#include "qt/QUISH/optionbutton.h"
#include "qt/QUISH/sendconfirmdialog.h"
#include "qt/QUISH/myaddressrow.h"
#include "qt/QUISH/guitransactionsutils.h"
...
SendWidget::SendWidget(QUISHGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (QTV)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a QUISH address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 QTV");
	...
	setCssProperty(coinIcon, "coin-icon-QTV");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_QTV, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any QTV to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress QTVAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(QTVAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(QTVAdd.Get(), dialog->getLabel().toStdString(),
				AddressBook::AddressBookPurpose::SEND)) {
