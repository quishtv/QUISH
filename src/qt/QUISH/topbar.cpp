...
#include "qt/QUISH/topbar.h"
#include "qt/QUISH/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/QUISH/lockunlock.h"
#include "qt/QUISH/qtutils.h"
#include "qt/QUISH/receivedialog.h"
...
TopBar::TopBar(QUISHGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopQTV}, "amount-small-topbar");
    setCssProperty(ui->labelAmountQTV, "amount-topbar");
    setCssProperty({ui->labelPendingQTV, ui->labelImmatureQTV }, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("QUISH")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // QTV Total
    CAmount QTVAvailableBalance = balance;

    // Set
    QString totalQTV = GUIUtil::formatBalance(QTVAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopQTV->setText(totalQTV);

    // Expanded
    ui->labelAmountQTV->setText(totalQTV);

    ui->labelPendingQTV->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureQTV->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
