...
#include "qt/QUISH/dashboardwidget.h"
#include "qt/QUISH/forms/ui_dashboardwidget.h"
#include "qt/QUISH/sendconfirmdialog.h"
#include "qt/QUISH/txrow.h"
...
DashboardWidget::DashboardWidget(QUISHGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of QTV staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareQTV, "square-chart-QTV");
    setCssProperty(ui->labelQTV, "text-chart-QTV");
    ...
    ui->labelAmountQTV->setText("0 QTV");
    setCssProperty(ui->labelAmountQTV, "text-stake-QTV-disable");
    ...
    ui->btnHowTo->setText(tr("How to get QTV"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("QTV")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            QTV = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalQTV += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesQTV.append(QTV);

        int max = std::max(QTV, QTV);
...
void DashboardWidget::onChartRefreshed() {
    ...
    set0->append(chartData->valuesQTV);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalQTV > 0) {
        setCssProperty(ui->labelAmountQTV, "text-stake-QTV");
    } else {
        setCssProperty(ui->labelAmountQTV, "text-stake-QTV-disable");
    }
    forceUpdateStyle({ui->labelAmountQTV});
    ui->labelAmountQTV->setText(GUIUtil::formatBalance(chartData->totalQTV, nDisplayUnit));
