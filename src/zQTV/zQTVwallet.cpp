...
CzQTVWallet::CzQTVWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zQTV seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZQTVSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZQTVSeed_deprecated()) {
                    LogPrintf("%s: Updated zQTV seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zQTV seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zQTV wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzQTVWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZQTVCount(nCountLastUsed);
    else if (!walletdb.ReadZQTVCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzQTVWallet::Lock()
{
    seedMaster = 0;
}

void CzQTVWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzQTVWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZQTV(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzQTVWallet::LoadMintPoolFromDB()
{
...
void CzQTVWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzQTVWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzQTVWallet::SyncWithChain(bool fGenerateMintPool)
{
...
void CzQTVWallet::SeedToZQTV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzQTVWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzQTVWallet::GenerateDeterministicZQTV(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzQTVWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzQTVWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzQTVWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
